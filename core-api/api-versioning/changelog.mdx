...
This means these changes won’t affect your integration unless you explicitly opt into the newer version (see [Targeting a specific version](/guides/api-versioning/usage)){/* TODO: link to ported spec */}.
...
- Add new bulk Dot Phrase management endpoints to the **User API**, each supporting up to 100 items per request:
  - [`POST /dot-phrases/bulk-create`](/user/bulk-create-user-dot-phrases){/* TODO: link to ported spec */} — Create multiple Dot Phrases in a single request.
  - [`POST /dot-phrases/bulk-update`](/user/bulk-update-user-dot-phrases) — Update multiple Dot Phrases in a single request.
  - [`POST /dot-phrases/bulk-delete`](/user/bulk-delete-user-dot-phrases) — Delete multiple Dot Phrases in a single request.
...
- Add new **User API endpoints** to allow users to configure and persist their note generation preferences:
  - [`GET /note-settings`](/user/get-note-settings){/* TODO: link to ported spec */} — Retrieve the user’s preferred **locale** and **template**.
    Default values: `note_locale = ENGLISH_US`, `note_template = GENERIC_MULTIPLE_SECTIONS`.
  - [`PATCH /note-settings`](/user/update-note-settings) — Update the user’s preferred **locale** and **template**.
  - [`GET /note-settings/note-sections-customization/:note_template`](/user/get-note-sections-customization) — Retrieve the default **per-section customization** options for a specific template.
  - [`PATCH /note-settings/note-sections-customization/:note_template`](/user/update-note-sections-customization) — Update the default **per-section customization** options for a specific template.
- Change in User API [`/generate-note`](/user/generate-note) and [`/generate-note-async`](/next/user/generate-note-async) endpoints:
...
- Change in User API [`/edit-note-with-instructions`](/user/edit-note-with-instructions) endpoint:
...
- Add a new [synchronous dictation from file upload endpoint](/server/dictate){/* TODO: link to ported spec */}.
- Add a new [asynchronous dictation from file URL](/server/dictate-async), with the ability to either:
  - poll the result through a [polling endpoint](/server/get-dictation-async);
...
- Add a new [audio chunk acknowledgement protocol](/guides/best-practices/transcription-network-resilience){/* TODO: link to ported spec */}, this change applies to all prior versions too.
...
- **New `2024-10-01` major API version. Check the [Migration guide](/guides/api-versioning/migrating-from-prior-to-2024-10-01){/* TODO: link to ported spec */} for an exhaustive list of the changes.**
- Add support for [OAuth Client authentication](/guides/authentication) flow for server-to-server integrations.
- Add support for [Webhooks](/webhook/api-receive-webhook) with events sent upon completion of asynchronous tasks like asynchronous note generation.
- Add a new [transcription websocket](/server/transcribe-ws) with a simpler frames schema, and **support for 35 languages**.
- Add a new [note generation endpoint](/server/generate-note) with **[per-section customization capabilities](/next/guides/note-templates/note-customization)**.
- Add a new [asynchronous note generation endpoint](/server/generate-note-async) with the ability to poll the result through a [polling endpoint](/next/server/get-generate-note-async).
- Add a new [transcription from file upload endpoint](/server/transcribe) with a simpler body schema.
- Add a new [transcription from file upload endpoint](/server/transcribe-async) with the ability to poll the result through a [polling endpoint](/next/server/get-transcribe-async).
...
- Add four new [templates](/guides/note-templates/note-templates-sections){/* TODO: link to ported spec */}: `GENERAL_MEDICINE_WCC`, `SOAP_WCC`, `GENERAL_MEDICINE_EMERGENCY`, `SOAP_EMERGENCY`.
